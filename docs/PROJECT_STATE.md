...
| Last **committed** milestone | **4.6 rate limiting — verified green on 2026-08-14** |
...
| Verified build | `.\mvnw.cmd clean verify` → **`Tests run: 69, Failures: 0, Errors: 0, Skipped: 0`**, `BUILD SUCCESS` |
| Next verified step | Phase 4.7 — password reset, the last authentication flow specification section 14 requires. What remains of Phase 4 after it: `PATCH /users/me`, change password, change email and confirm, and `DELETE /users/me` |
...
`AuthServiceTest` replaces `EmailProvider` with `@MockitoBean` and
`AuthRateLimitTest` overrides the rate limit properties. Each change to the
context configuration buys a separate Spring context and therefore a separate
PostgreSQL container: the build now starts **four**.
