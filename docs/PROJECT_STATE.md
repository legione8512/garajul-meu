...
| Item | Value |
|---|---|
| Phase | **4 — Authentication & Users: complete. 5 — Frontend Foundation & i18n: complete.** 6 — Dashboard & Garage Skeleton: **in progress**, 6.1 done |
| Last **committed** milestone | **Phase 5.4c — `64a320d`**, which closed Phase 5. Note that `11ed206` carries the same subject line: the first commit was made before the `LoginPage` handover fix found by the browser walkthrough |
| Applied, verified green, **not yet committed** | **Phase 6.1** — route protection and the authenticated shell: `RequireAuth`, `AppLayout`, `SiteHeader`, the three destination screens as placeholders, and `returnTo`. Also a real fix to `refresh.ts`, described in the decisions table |
...
| `src/pages/` | Screens 1–5 of section 5 plus not-found, all real. Welcome changes with the authentication status; the other four are forms |
...
| `pages/LoginPage.test.tsx` (6) | An empty form is refused without a request; signing in lands on the welcome page; refused credentials are translated; **a server field error lands on its own field via `aria-describedby`**; the inputs carry the autocomplete hints a password manager needs; the address handed over by verification or reset pre-fills |
| `pages/WelcomePage.test.tsx` (3) | Both ways in when signed out; a greeting and a way out when signed in; **neither while the session is still being restored** |
