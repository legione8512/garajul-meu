...
| Item | Value |
|---|---|
| Phase | **4, 5, 6 and 7 complete** once 7.3c is committed. 8 — the full registration certificate — has not started |
| Last **committed** milestone | **Phase 7.3b — `3ca718a`**: the add-vehicle screen (section 5 screen 8), the `/garage/new` route, `createVehicle`, and the garage's link to it |
| Applied, verified green, **not yet committed** | **Phase 7.3c** — the vehicle detail screen (section 5 screen 12) on `/garage/:vehicleId`, renaming through `PATCH`, deletion behind an inline confirmation, the garage rows as links, and the add screen redirecting to the vehicle it just created. **The vehicle domain is now whole end to end**: create, list, read, rename, delete, all ownership-scoped |
...
| Verified build — backend | `.\mvnw.cmd clean verify` → **`Tests run: 127, Failures: 0, Errors: 0, Skipped: 0`** across **20 test classes**, `BUILD SUCCESS`, **four** PostgreSQL containers. Count the classes as well as the tests: `ls target/surefire-reports/*.txt \| wc -l`. A class's elapsed time also reveals whether it started a container of its own — under a second means it reused a cached context, twenty-odd seconds means a fifth container appeared |
| Verified build — frontend | from `frontend/`: `npm run test:run` → **113 passed in 25 test files**; `npm run build`; `npm run lint`. **Count the files as well as the tests**, and check for empty ones with `find src -type f -size -1c` — on 2026-08-14 a test file never reached disk and on 2026-08-15 two arrived empty, all three invisible to a green suite |
| Next verified step | **A browser walkthrough of the whole vehicle flow, before Phase 8 begins.** The standing rule from 5.4 applies and has earned itself once already: walking two screens in a row found a defect that seventy-nine tests did not, because every test renders one screen with its inputs handed to it directly, which is exactly the shape that hides anything living *between* screens. Add a vehicle, find it in the garage, open it, rename it, clear the nickname, delete it — against both servers running. Then **Phase 8 — the full registration certificate**: the thirty optional columns, the field policy in section 8, and the overlay template in section 7, whose normalised coordinates are calibrated in that phase and never guessed. Section 5 screens **7 My Garage, 8 Add Vehicle and 12 Vehicle Details** are done — 11 is Configure Documents & Expiries and belongs to Phase 10. Section 3 calls manual creation a first-class path — "Vehicle creation manually or from a photographed Romanian registration certificate" — so the Phase 7 form is a finished feature, not a placeholder waiting for the scan path in Phase 9. Section 6 sets the i18n contract — `ro` and `en`, no user-facing string hardcoded in a component, the pre-authentication language kept in a local non-sensitive preference and the authenticated one in `users.preferred_language`, and **the frontend translating the backend's stable error codes**. Section 5 lists twenty-two screens; 1–5 were Phase 5, and the skeletons of the dashboard, the garage and the profile were Phase 6. The garage and vehicle screens with real content are 7.3. |
...
| `src/components/TextField.tsx` | Label, input and message wired together — `aria-invalid`, `aria-describedby`, a generated id. **The accessibility wiring exists once** rather than five times, which matters because `eslint-plugin-jsx-a11y` cannot be installed to check it |
...
`AccountDeletionTest`, `VehicleReadTest` and `VehicleWriteTest` match
`AuthFlowTest`. Four of those — `UserProfileFlowTest`, `AccountDeletionTest`,
`VehicleReadTest` and `VehicleWriteTest` — do so **with an `EmailProvider` mock
...
### Frontend tests — 113 in 25 files
