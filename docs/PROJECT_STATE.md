...
| Last **committed** milestone | **Phase 7.2 — `52f3aee`**: `POST /api/v1/vehicles` writing the vehicle and its certificate in one transaction, `PATCH` for the nickname, `DELETE`, VIN normalisation, and the duplicate-VIN rule enforced by both a check and the unique index |
| Applied, verified green, **not yet committed** | **Phase 7.3a** — `useResource`, the vehicles endpoint module, and the garage listing real data with loading, failed, empty and retry states. Rows are **not yet links**: the detail screen arrives in 7.3c, and a link to a route that does not exist is a worse intermediate state than a row that is not yet clickable |
...
| `src/pages/` | Screens 1–5 of section 5 plus not-found on the public side, and screens 6, 7 and 15 behind the gate. Welcome is the public landing and redirects anyone signed in to the dashboard; the dashboard and garage carry empty states with **no call to action**, because adding a vehicle needs a confirmed certificate and that is Phase 8; the profile shows the account and ends the session |
...
| `src/api/endpoints/` | `auth.ts` — `login`, `logout`; `users.ts` — `getProfile`; `vehicles.ts` — the vehicle types, the collection **path** and `vehicleLabel`. It exports a path rather than a fetcher, unlike its neighbours, because `useResource` keys its effect on a string and a function would be a new value every render |
...
### Frontend tests — 101 in 23 files
...
| `pages/GaragePage.test.tsx` (4) | The vehicles are listed, **by nickname where there is one and by make and description where there is not** — both labelling rules in one test, because the rule *is* the choice between them. The empty garage says so. A refused request shows translated wording rather than nothing, and retrying brings the list back — recovery, not merely a second call |
