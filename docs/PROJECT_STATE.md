...
| `pages/LoginPage.test.tsx` (5) | An empty form is refused without a request; signing in lands on the welcome page; refused credentials are translated; **a server field error lands on its own field via `aria-describedby`**; the inputs carry the autocomplete hints a password manager needs |
