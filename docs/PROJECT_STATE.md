...
| Item | Value |
|---|---|
| Phase | **4 — Authentication & Users: complete. 5 — Frontend Foundation & i18n: complete** once 5.4c is committed. 6 — Dashboard & Garage Skeleton has not started |
| Last **committed** milestone | **Phase 5.4b — `8ef996e`**: the sign-in form and the status-aware welcome screen |
| Applied, verified green, **not yet committed** | **Phase 5.4c** — the remaining four screens, plus `TextField` and `carriedEmail`. `LoginPage` was rewritten onto `TextField` in the same step so only one pattern exists |
...
| `src/routes/carriedEmail.ts` | Reads an address handed over in route state — never a query parameter, which would put it in history and in every proxy log |
...
| `routes/carriedEmail.test.ts` (3) | An address handed over is read back; absent state and a non-string value both yield nothing — the reload case every screen has to survive |
