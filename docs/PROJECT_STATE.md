...
| Item | Value |
|---|---|
| Phase | **4, 5 and 6 complete. 7 — Vehicle Domain in progress: the whole backend half (7.1 and 7.2) is done; 7.3 is the frontend** |
| Last **committed** milestone | **Phase 7.1 — `f37baaf`**: the `vehicles` and `registration_certificates` tables, both entities, ownership enforced **inside** the repository queries, `GET /api/v1/vehicles` and `GET /api/v1/vehicles/{id}`, and a handler for a path parameter that will not convert |
| Applied, verified green, **not yet committed** | **Phase 7.2** — `POST /api/v1/vehicles` writing the vehicle and its certificate in one transaction, `PATCH` for the nickname, `DELETE`, VIN normalisation, and the duplicate-VIN rule enforced by both a check and the unique index |
...
| `pages/DashboardPage.test.tsx` (1), `pages/GaragePage.test.tsx` (1) | Each says it has nothing to show. Nothing is fetched: the vehicle domain does not exist until Phase 7 |
