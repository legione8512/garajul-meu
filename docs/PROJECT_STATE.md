...
| Item | Value |
|---|---|
| Phase | **4, 5 and 6 complete. 7 — Vehicle Domain in progress: the backend half is done and committed; 7.3a of the frontend half is applied** |
| Last **committed** milestone | **Phase 7.3a — `8e01b71`**: `useResource`, the vehicles endpoint module, and the garage listing real data with loading, failed, empty and retry states |
| Applied, verified green, **not yet committed** | **Phase 7.3b** — the add-vehicle screen (section 5 screen 8), the `/garage/new` route, `createVehicle`, and the garage's link to it. Garage rows are still **not links**: the detail screen arrives in 7.3c, and a link to a route that does not exist is a worse intermediate state than a row that is not yet clickable. For the same reason a successful submission lands on the garage rather than on the new vehicle's own screen — **7.3c changes that one line** |
...
| Verified build — frontend | from `frontend/`: `npm run test:run` → **106 passed in 24 test files**; `npm run build`; `npm run lint`. **Count the files as well as the tests**, and check for empty ones with `find src -type f -size -1c` — on 2026-08-14 a test file never reached disk and on 2026-08-15 two arrived empty, all three invisible to a green suite |
| Next verified step | **Phase 7.3c — the vehicle detail screen** (section 5 screen 12): the `/garage/:vehicleId` route, renaming through `PATCH`, deletion with an **inline** confirmation rather than `window.confirm` — whose wording comes from the browser's language, not the application's, the same argument that puts `noValidate` on every form — the garage rows becoming links, and the add screen redirecting to the new vehicle instead of the garage. Section 5 screens **7 My Garage, 8 Add Vehicle and 12 Vehicle Details** — 11 is Configure Documents & Expiries and belongs to Phase 10. Section 3 calls manual creation a first-class path — "Vehicle creation manually or from a photographed Romanian registration certificate" — so the Phase 7 form is a finished feature, not a placeholder waiting for the scan path in Phase 9. Phase 8 then widens the certificate to its remaining thirty optional columns with the field policy in section 8. Section 6 sets the i18n contract — `ro` and `en`, no user-facing string hardcoded in a component, the pre-authentication language kept in a local non-sensitive preference and the authenticated one in `users.preferred_language`, and **the frontend translating the backend's stable error codes**. Section 5 lists twenty-two screens; 1–5 were Phase 5, and the skeletons of the dashboard, the garage and the profile were Phase 6. The garage and vehicle screens with real content are 7.3. |
...
| `src/pages/` | Screens 1–5 of section 5 plus not-found on the public side, and screens 6, 7, 8 and 15 behind the gate. Welcome is the public landing and redirects anyone signed in to the dashboard; the garage lists real vehicles and offers the way to add one; the dashboard still carries an empty state, because `GET /api/v1/dashboard` waits for Phase 10; the profile shows the account and ends the session |
...
| `src/api/endpoints/` | `auth.ts` — `login`, `logout`; `users.ts` — `getProfile`; `vehicles.ts` — the vehicle types, `createVehicle`, `vehicleLabel`, and the collection **path**. Reads are exposed as a path and writes as functions, which looks inconsistent and is not: `useResource` keys its effect on a string, and a fetcher would be a new value every render, while a write is called once from an event handler and has no such problem |
...
### Frontend tests — 106 in 24 files
